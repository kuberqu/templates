...
def build(image: str, prompt: str, frames: int, w: int, h: int, seed: int,
          prefix: str, strength: float, audio: str | None = None,
          idg: float = 3.0) -> dict:
    """audio gesetzt = Host-Szene: LTXVReferenceAudio koppelt die Lippen an die
    Narration dieser Szene. Ohne Audio (B-Roll) entsteht die Bewegung nur aus dem
    Prompt - bei sprechenden Szenen ist der Mund dann NICHT synchron zum Text,
    was im fertigen Short sichtbar auffaellt."""
    bw, bh = w // 2, h // 2
    g = {
    ...
    if audio:
        g["40"] = {"class_type": "LoadAudio", "inputs": {"audio": audio}}
        g["41"] = {"class_type": "LTXVReferenceAudio",
                   "inputs": {"model": ["1", 0], "positive": ["5", 0], "negative": ["5", 1],
                              "reference_audio": ["40", 0], "audio_vae": ["17", 0],
                              "identity_guidance_scale": idg,
                              "start_percent": 0.0, "end_percent": 1.0}}
        for nid in ("9", "22"):
            g[nid]["inputs"]["model"] = ["41", 0]
            g[nid]["inputs"]["positive"] = ["41", 1]
            g[nid]["inputs"]["negative"] = ["41", 2]
    return g
...
def main() -> None:
    ...
    for sz in cfg["szenen"]:
        ...
        # Host-Szenen sprechen: die Narration DIESER Szene als Referenzaudio
        # mitgeben, sonst ist der Mund nicht synchron zum Text (LoadAudio liest
        # wie LoadImage nur aus ComfyUI/input).
        audio_name = None
        if sz["typ"] == "host":
            audio_name = f"short_{os.path.basename(base)}_szene_{sz['id']:02d}.wav"
            shutil.copyfile(os.path.join(base, "audio", f"szene_{sz['id']:02d}.wav"),
                            os.path.join("/workspace/ComfyUI/input", audio_name))
